...
type *arg = malloc(sizeof(type));  \
*((type *) output) = (type) *(int *) initial_value; \
for (size_t i = 0; i < n; ++i) {                    \
    *arg = va_arg(args, type); \
    output = opr(output, arg); \
}                            \
if (opr == meanInt || opr == meanDouble) {    \
    *((type *) output) = *((type *) output) / n; \
}

#define apply(type, init, func) vaggregate(sizeof(type), init, func, ARR_SIZE, type ## s)

#define ints 1, 2, 3, 4, 5
#define doubles 3.14, 0.5, 8.08, 1.1, 10.0

void *vaggregate(size_t size, void *initial_value, void *(*opr)(const void *, const void *), int n, ...);
